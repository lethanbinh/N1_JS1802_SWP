...
public interface OrderStatisticsRepository extends JpaRepository<Order, Integer> {
    ...
    @Query("SELECT AVG(o.totalPrice) FROM Order o WHERE o.createDate BETWEEN :startDate AND :endDate AND o.type = 'SELL'")
    Double findAverageOrderValue(@Param("startDate") Date startDate, @Param("endDate") Date endDate);

    @Query("SELECT SUM(o.totalPrice) FROM Order o WHERE o.createDate BETWEEN :startDate AND :endDate AND o.type = 'SELL'")
    ...
    @Query("SELECT FUNCTION('YEAR', o.createDate) AS year, FUNCTION('MONTH', o.createDate) AS month, COUNT(o) " +
            "FROM Order o WHERE o.createDate BETWEEN :startDate AND :endDate AND o.type = 'SELL' " +
    ...
    @Query("SELECT (YEAR(CURRENT_DATE) - YEAR(c.birthday)) - (CASE WHEN MONTH(CURRENT_DATE) < MONTH(c.birthday) OR (MONTH(CURRENT_DATE) = MONTH(c.birthday) AND DAY(CURRENT_DATE) < DAY(c.birthday)) THEN 1 ELSE 0 END) AS age, COUNT(o) " +
            "FROM Order o JOIN Customer c ON o.customer.id = c.id WHERE o.createDate BETWEEN :startDate AND :endDate AND o.type = 'SELL' " +
            "GROUP BY age")
    ...
    @Query(value = "SELECT AVG(order_count) " +
            ...
            "      FROM orders o " +
            "      WHERE o.create_date BETWEEN :startDate AND :endDate AND o.type = 'SELL' " +
